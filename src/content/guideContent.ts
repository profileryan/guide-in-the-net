...
export const sectionTwoArtworks: Record<Extract<ArtworkId, 'altar' | 'traces'>, ArtworkContent> = {
  ...
  traces: {
    ...
    description: [
      'Traces in the Net is a speculative platform that extends outward from the exhibition space of Islands in the Net, to encompass the island of Singapore and beyond.',
      'Users are invited to leave voice notes—inscribing emotions, confessions, or whatever else they find meaningful—as acoustic traces tied to specific locations. These can be revisited or discovered by others via a simple webapp. They might be temporary, and decay over a given time. The record is deliberately partial: a constellation of intimate voices that continues to expand. Some may remain; others may eventually disappear.',
      'Behind the scenes, a blockchain archive stores only a timestamp proving that a feeling once existed here.',
      'The project is an experiment in cumulative, decentralised data preservation, and the possibilities of oral history and organic counter-cartography. It attempts to address the fact that data has rarely had a precise relationship to urban space. And, conversely, that our architectures and cities have been amnesiac: lacking a mechanism to preserve the personal histories, events, and affective states of those who live their lives within. At the same time, it resists the convention by which only official histories have the power and permanence to survive over time.',
      'Traces invites individuals to find resonances and form communities through the ways they move through the world. In doing so, the project points to how human experience today is fundamentally entangled across physical and virtual planes.',
    ],
    whyNow: [
